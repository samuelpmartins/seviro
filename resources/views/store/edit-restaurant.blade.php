                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <h2 class="card-title mb-0">{{ $categories->sum(function($cat) { return $cat->products->count(); }) }}</h2>
                            </div>
        ...
        <div class="accordion" id="editMenuAccordion">
            ...
            <div class="accordion-item">
                <h2 class="accordion-header">
                    <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseEstablishment">
                        <i class="fas fa-store me-2"></i> Estabelecimento
                    </button>
                </h2>
                <div id="collapseEstablishment" class="accordion-collapse collapse show" data-bs-parent="#editMenuAccordion">
                    <div class="accordion-body">
                        <div class="accordion-content">
                            <div class="establishment-info">
                                @if($store->logo)
                                    <img src="{{ asset('storage/' . $store->logo) }}" alt="{{ $store->name }}" class="establishment-logo">
                                @else
                ...
                <div id="collapseProducts" class="accordion-collapse collapse" data-bs-parent="#editMenuAccordion">
                    <div class="accordion-body">
                        <div class="accordion-content">
                            ...
                            @forelse($categories as $category)
                                <div class="category-item" data-id="{{ $category->id }}">
                                    <div class="category-header">
                                        <i class="fas fa-grip-vertical text-muted cursor-move me-2" style="cursor: move;"></i>
                                    ...
                                    @if($category->products->count() > 0)
                                        <div class="product-list sortable-products" data-category-id="{{ $category->id }}">
                                            @foreach($category->products as $product)
                                                <div class="product-item" data-id="{{ $product->id }}">
                                                    <i class="fas fa-grip-vertical text-muted cursor-move me-2" style="cursor: move; font-size: 0.8rem;"></i>
                                                    @if($product->image)
                                                    ...
                                                    @if($product->is_quick_item)
                                                        <span class="badge bg-info me-1" title="Item rápido - entrega direta pelo garçom">
                                    ...
                                    @else
                                        <div class="empty-state" style="padding: 15px;">
                                            <p style="font-size: 0.8rem; margin: 0;">Nenhum produto nesta categoria</p>
                                        </div>
                                    @endif
                                </div>
                            @empty
                            ...
                            <button type="button" class="btn action-btn action-btn-success" data-bs-toggle="modal" data-bs-target="#newCategoryModal">
                                <i class="fas fa-plus-circle me-2"></i> Nova Categoria
                            </button>
                            <button type="button" class="btn action-btn action-btn-primary" data-bs-toggle="modal" data-bs-target="#newProductModal">
            ...
            <div class="accordion-item">
                <h2 class="accordion-header">
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTables">
                ...
                <div id="collapseTables" class="accordion-collapse collapse" data-bs-parent="#editMenuAccordion">
                    <div class="accordion-body">
                        <div class="accordion-content">
                            @if($tables->count() > 0)
                                <div class="table-grid">
                                    @foreach($tables as $table)
                                        <a href="{{ route('store.tables.edit', $table->id) }}" style="text-decoration: none;">
                                            <div class="table-card {{ $table->occupied ? 'occupied' : 'available' }}">
                                                <p class="table-number">Mesa {{ $table->number }}</p>
                                                <p class="table-status {{ $table->occupied ? 'occupied' : 'available' }}">
                            ...
                            <button type="button" class="btn action-btn action-btn-success" data-bs-toggle="modal" data-bs-target="#newTableModal">
                        ...
                        <div class="accordion-content">
                            ...
                            <div class="filter-section">
                                <div class="filter-input">
                                    <input type="text" id="orderFilter" placeholder="Buscar por mesa ou valor..." onkeyup="filterOrders()">
                                    ...
                                    <div class="order-item" 
                                        ...
                                        <div class="order-header">
                                            <span class="order-table-number">
                                                <i class="fas fa-receipt me-1"></i> {{ $order->order_number }} - {{ $order->table ? 'Mesa ' . $order->table->number : 'Balcão' }}
                                            </span>
                                        ...
                                        <div class="order-info">
                                            ...
                                            <span class="badge bg-{{ $order->status === 'Aguardando pagamento' ? 'warning' : 
                                                                     ...
                                                                     ($order->status === 'Finalizado' ? 'success' : 'primary')) }}">
                                                {{ $order->status }}
                                            </span>
                                        </div>
                                    </div>
                                            ...
                                            <div class="modal-content" style="border-radius: 15px; border: none;">
                                                <div class="modal-header" style="border-bottom: 2px solid #ecf0f1;">
                                                    <h5 class="modal-title" style="color: #2c3e50; font-weight: 700;">
                                                        <i class="fas fa-receipt me-2"></i> Pedido {{ $order->order_number }} - {{ $order->table ? 'Mesa ' . $order->table->number : 'Balcão' }}
                                                    </h5>
                                                ...
                                                <div class="modal-body" style="padding: 25px;">
                                                    ...
                                                    <div class="mb-3">
                                                        <strong style="color: #2c3e50;">Data:</strong> {{ $order->created_at->format('d/m/Y H:i') }}
                                                    </div>
                                                    @if($order->notes)
                                                        <div class="mb-3">
                                                            <strong style="color: #2c3e50;">Observações:</strong>
                                                            <p class="mb-0" style="color: #7f8c8d;">{{ $order->notes }}</p>
                                                        </div>
                                                    @endif
                                                    <div class="mb-3">
                                                        ...
                                                        <div style="margin-top: 10px;">
                                                            @foreach($order->items as $item)
                                                                <div style="display: flex; align-items: center; gap: 10px; padding: 10px; background: #f8f9fa; border-radius: 8px; margin-bottom: 8px;">
                                                                    ...
                                                                    @endif
                                                                    <div style="flex: 1;">
                                                                        <strong style="color: #2c3e50;">{{ $item->product->name }}</strong>
                                                                        <br>
                                                    ...
                                                    <div style="text-align: right; padding-top: 15px; border-top: 2px solid #ecf0f1; margin-top: 15px;">
                                                        <strong style="color: #2c3e50; font-size: 1.2rem;">
                                                            Total: <span style="color: #27ae60;">R$ {{ number_format($order->total, 2, ',', '.') }}</span>
                                                        </strong>
                                                    </div>
                                                </div>
...
<div class="modal fade" id="newProductModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form id="newProductForm" action="{{ route('store.products.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
                    ...
                    <div class="form-check mb-2">
                        <input type="checkbox" class="form-check-input" id="product_active" name="active" value="1" checked>
                        <label class="form-check-label" for="product_active">
                            Produto ativo
                        </label>
                    </div>
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="product_is_quick_item" name="is_quick_item" value="1">
                        <label class="form-check-label" for="product_is_quick_item">
                            Item rápido
                        </label>
                        <small class="d-block text-muted">Itens rápidos (ex: refrigerantes) não vão para a cozinha, são entregues diretamente pelo garçom.</small>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div id="current_image_preview" class="mt-2" style="display: none;">
                            ...
                            <img id="current_image" src="" alt="Imagem atual" class="img-thumbnail" style="max-height: 100px;">
                        </div>
                    </div>
                    <div class="form-check mb-2">
                        <input type="checkbox" class="form-check-input" id="edit_product_active" name="active" value="1">
                        <label class="form-check-label" for="edit_product_active">
                            Produto ativo
                        </label>
                    </div>
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="edit_product_is_quick_item" name="is_quick_item" value="1">
