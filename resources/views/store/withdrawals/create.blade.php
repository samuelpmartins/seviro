        ...
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    @if($bankAccount->hasPixData())
                        <div class="mb-3">
                            ...
                            <span class="text-muted">{{ $bankAccount->pixKeyTypeName }}:</span> {{ $bankAccount->pix_key }}
                        </div>
                    @endif
                    ...
                    @if($bankAccount->hasBankData())
                        ...
                        <div>
                            ...
                            <span class="text-muted">Banco:</span> {{ $bankAccount->bank_code }} - {{ $bankAccount->bank_name }}<br>
                            <span class="text-muted">Agência:</span> {{ $bankAccount->agency }}<br>
                            <span class="text-muted">Conta:</span> {{ $bankAccount->account_number }}-{{ $bankAccount->account_digit }} ({{ $bankAccount->accountTypeName }})<br>
            ...
            <form action="{{ route('store.withdrawals.store') }}" method="POST" id="withdrawalForm">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <small class="form-text text-muted">Valor máximo: R$ {{ number_format($balance, 2, ',', '.') }}</small>
                        </div>
                ...
                <div class="alert alert-info">
                    <strong>Importante:</strong> A solicitação passará por análise do administrador antes de ser processada. Você será notificado sobre o status.
                </div>
