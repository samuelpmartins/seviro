    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-success text-white">
                <h5 class="modal-title"><i class="fas fa-cash-register me-2"></i>Receber Pagamento - Mesa <span id="tableNumber"></span></h5>
            ...
            <div class="modal-body">
                ...
                <div id="paymentMethodSection" class="mt-4">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-4">
                            <div class="form-check payment-type-card" style="cursor: pointer;">
                                <input class="form-check-input" type="radio" name="paymentTypeService" id="paymentTypeCash" value="cash" checked>
                        ...
                        <div class="col-4">
                            <div class="form-check payment-type-card" style="cursor: pointer;">
                                <input class="form-check-input" type="radio" name="paymentTypeService" id="paymentTypeCard" value="card">
                        ...
                        <div class="col-4">
                            <div class="form-check payment-type-card" style="cursor: pointer;">
                                <input class="form-check-input" type="radio" name="paymentTypeService" id="paymentTypePix" value="pix">
                    ...
                    <div class="mt-3">
                        ...
                        <textarea class="form-control" id="paymentNotes" rows="2" placeholder="Alguma observacao sobre o pagamento..."></textarea>
                    </div>
                </div>
...
<style>
    ...
    [data-bs-theme="dark"] .table-card.free .status-text {
        color: #75b798 !important; /* Verde claro para tema escuro */
    }
    
    [data-bs-theme="dark"] .table-card.occupied .status-text {
        color: #ea868f !important; /* Vermelho claro para tema escuro */
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tableCards.forEach(card => {
        ...
        card.addEventListener('click', function(e) {
            ...
            document.getElementById('clearTableOption').style.display = isOccupied ? 'block' : 'none';
            
            // Configurar link de ver pedidos
            const viewOrdersLink = document.getElementById('viewOrdersLink');
            viewOrdersLink.href = '/store/orders/history?table_id=' + tableId + '&start_date=' + new Date().toISOString().split('T')[0];
            
            // Mostrar/esconder opção de pagamento
            document.getElementById('paymentOption').style.display = unpaidTotal > 0 ? 'block' : 'none';
    ...
    function updateSelectedTotalService() {
        document.getElementById('selectedTotalService').textContent = 'R$ ' + selectedTotalService.toLocaleString('pt-BR', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
        });
    ...
    document.getElementById('cashPaymentForm')?.addEventListener('submit', function(e) {
        ...
        if (cashReceived < selectedTotalService) {
            alert('O valor recebido deve ser maior ou igual ao total: R$ ' + selectedTotalService.toLocaleString('pt-BR', {minimumFractionDigits: 2}));
            return;
        }

        // Preencher campos hidden
        document.getElementById('cashReceivedHidden').value = cashReceived;
        document.getElementById('paymentNotesHidden').value = document.getElementById('paymentNotes').value;
    ...
    function loadUnpaidOrders(tableId) {
        ...
        // Mostrar loading
        ordersList.innerHTML = '<div class="text-center py-3"><div class="spinner-border text-primary" role="status"></div></div>';
        
        fetch('/api/tables/' + tableId + '/unpaid-orders')
            .then(response => response.json())
            .then(data => {
                if (data.orders.length === 0) {
                    ordersList.innerHTML = '<div class="alert alert-info mb-0">Nenhum pedido pendente encontrado.</div>';
                    return;
                }
            ...
            .catch(error => {
                ordersList.innerHTML = '<div class="alert alert-danger mb-0">Erro ao carregar pedidos.</div>';
                console.error(error);
            });
    }
