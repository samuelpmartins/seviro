...
@section('content')
                ...
                <div class="modal-content" style="border-radius: 16px; max-width: 420px; width: 100%;">
                    ...
                    <div class="modal-body" style="padding: 2rem;">
                        ...
                        <div id="createPasswordStep" class="d-none">
                            ...
                            <p class="text-center mb-3">Digite seu nome. Um código de acesso será gerado para que você
                                acesse esta mesa
                                novamente.</p>
                        ...
                        <div id="pinNoticeStep" class="d-none text-center">
                            <p class="mb-3">Seu código de acesso de identificação da mesa foi gerado.</p>
                            <p class="mb-4">Use esse código de acesso para acessar a mesa novamente. Se fechar o
                                navegador, você
                        ...
                        <div id="validatePinStep" class="d-none">
                            ...
                            <p class="text-center mb-3">Digite o código de acesso que foi gerado para você anteriormente.
                            </p>
                            <form id="validatePinForm">
                                <div class="mb-3">
                                    <label for="tablePin" class="form-label">Código de acesso da Mesa</label>
                                ...
                                <button type="submit" class="btn w-100"
                                    style="background: #000; color: white; border-radius: 8px; padding: 0.75rem; font-weight: 600;">
                                    <i class="fas fa-unlock me-2"></i>Validar código de acesso
                                </button>
                            </form>
    ...
    <style>
        ...
        @media (max-width: 576px) {
            ...
            .menu-header>.position-absolute .btn {
                flex: 1 1 auto;
                min-width: 0;
                max-width: 100%;
            }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('add-to-cart-btn')?.addEventListener('click', function() {
                ...
                const finalNotes = mergeNotesWithSummary(notes, summary);
            ...
            function mergeIngredientDiffs(items) {
                const byName = new Map();
                (items || []).forEach(item => {
                    const cleanedName = String(item.name || '').trim();
                    ...
                    byName.get(key).diff += Number(item.diff || 0);
                });

                return Array.from(byName.values()).filter(item => Number(item.diff) > 0);
            }
            ...
            function updateIngredientFromInput(id) {
                ...
                const added = mergeIngredientDiffs((currentProduct.selectedIngredients || []).filter(i => (parseInt(
                    i.selectedAmount) || 0) > (parseInt(i.amount_item) || 0)).map(i => ({
                    name: i.name,
                    diff: (parseInt(i.selectedAmount) || 0) - (parseInt(i.amount_item) || 0)
                })));
                const removed = mergeIngredientDiffs((currentProduct.selectedIngredients || []).filter(i => (
                    parseInt(i.selectedAmount) || 0) < (parseInt(i.amount_item) || 0)).map(i => ({
                    name: i.name,
            ...
            function generateObservationText(addedArr, removedArr) {
                const parts = [];
                const uniqueAdded = mergeIngredientDiffs(addedArr || []);
                const uniqueRemoved = mergeIngredientDiffs(removedArr || []);

                if (uniqueAdded.length) parts.push('Adicionados: ' + uniqueAdded.map(a =>
                    `${a.name} x${a.diff}`).join('; '));
                if (uniqueRemoved.length) parts.push('Removidos: ' + uniqueRemoved.map(r =>
                    `${r.name} x${r.diff}`).join('; '));
                return parts.join(' | ');
            }

            function mergeNotesWithSummary(notes, summary) {
                const seen = new Map();
                const addUniquePart = (value) => {
                ...
                (notes || '').split('|').forEach(addUniquePart);
                (summary || '').split('|').forEach(addUniquePart);
