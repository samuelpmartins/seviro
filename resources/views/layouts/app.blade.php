            ...
            <div class="container">
                @if(isset($logoClickable) && !$logoClickable)
                    <span class="navbar-brand" style="cursor: default;">
                        @auth
                            @role('store')
                                <img src="{{ asset('storage/img/logo.png') }}" alt="{{ auth()->user()->store->name }}" style="height: 36px; width: auto; object-fit: contain;">
                            @else
                                <img src="{{ asset('storage/img/logo.png') }}" alt="{{ config('app.name', 'Laravel') }}" style="height: 36px; width: auto; object-fit: contain;">
                            @endrole
                        @else
                            <img src="{{ asset('storage/img/logo.png') }}" alt="{{ config('app.name', 'Laravel') }}" style="height: 36px; width: auto; object-fit: contain;">
                        @endauth
                ...
                @else
                    <a class="navbar-brand" href="@auth @role('store') {{ route('store.manage') }} @else {{ url('/') }} @endrole @else {{ url('/') }} @endauth">
                        @auth
                            @role('store')
                                <img src="{{ asset('storage/img/logo.png') }}" alt="{{ auth()->user()->store->name }}" style="height: 36px; width: auto; object-fit: contain;">
                            @else
                    ...
                    <ul class="navbar-nav me-auto">
                        @auth
                            @role('admin')
                                ...
                                <li class="nav-item">
                                    <a class="nav-link" href="{{ route('admin.withdrawals.index') }}">Saques</a>
                                </li>
                            @endrole
                            @role('store')
                                <li class="nav-item">
                        ...
                        @else
                            <li class="nav-item dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                                    @auth
                                        <a class="dropdown-item d-flex align-items-center" href="{{ route('orders.history') }}">
